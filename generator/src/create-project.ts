...
Next steps:
- bun install
- cp .env.example .env  # copy env template (LOG_LEVEL defaults to info)${databaseSteps}
- bun run dev
`;
...
function printSummary(plan: ProjectPlan, outPath: string): void {
  const lines = [
    ...
    "next steps: bun install; cp .env.example .env; bun run dev",
  ];
  if (plan.features.includes("persistence")) {
    lines.push("  with persistence: bun run db:up; bun run db:migrate (run after cp, before dev)");
  }
