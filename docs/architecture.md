...
- **Docker:** imagen multi-stage sobre `oven/bun:1.3.14-slim` que instala el workspace completo con `--frozen-lockfile`, usuario no-root `bun`, labels OCI versionables (`IMAGE_VERSION` default 0.10.0, `IMAGE_SOURCE`), `STOPSIGNAL SIGTERM` y `APP_VERSION` desde el build; `.dockerignore` endurecido. Perfiles compose: `core` (API), `database` (postgres), `worker` (outbox worker). Los perfiles redis/storage/observability se omiten a propósito: sin servicios sin implementación (puntos de extensión en ADR-0008/ADR-0009).
